...
/**   下载更新文件   @author yanglei   @version [1.0, 2019.3]  */public class UpdateService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		notification.tickerText = getString(R.string.appName) + "更新";
		...
		new Thread() {
			public void run() {
				try {
					...
					File rootFile;
					if (is != null) {
						rootFile = new File("/sdcard/cloud");
						if (!rootFile.exists() && !rootFile.isDirectory())
							rootFile.mkdir();

						tempFile = new File("/sdcard/cloud/"
