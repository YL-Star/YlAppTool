package com.yl.module_base_utils.Camera.camerasurfaceview;
...
//            mParam.setPictureSize(pictureSize.width, pictureSize.height);
            ...
            mParam.setPreviewSize(optionSize.width, optionSize.height);//把camera.size赋值到parameters
            int yuv_buffersize = optionSize.width * optionSize.height * ImageFormat.getBitsPerPixel(previewformat) / 8;
            previewBuffer = new byte[yuv_buffersize];
...
//                mCamera.setPreviewCallback(previewCallback);//设置摄像头预览帧回调
     ...
     **/
    CamerSurfaceInf camerSurfaceInf;


    public void capture(CamerSurfaceInf camerSurfaceInf) {
        this.camerSurfaceInf = camerSurfaceInf;
        if (mCamera == null) return;
            ...
            try {
                mCamera.takePicture(null, null, new Camera.PictureCallback() {
                    ...
                    public void onPictureTaken(byte[] data, Camera camera) {
                        ...
                        Bitmap face = serviceFile.getZoomImage(bitmap, 100);
//                        serviceFile sf = new serviceFile();
//                        String baseImag= Base64Util.encode(sf.Bitmap2Bytes(face));

//                        String baseImag=  Base64Util.encode(data);
                        camerSurfaceInf.Imgcallback(myUtils.getFile(face));
